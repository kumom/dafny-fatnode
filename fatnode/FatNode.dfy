class Node {
  ...
  ghost function ValueSetsAtVersion(version: int) : (res: (int, set<int>))
    ...
    requires version >= 0
    requires Valid()
    ensures res.0 >= 0 ==> res.1 in ValueSets
    ...
    ensures ValueSetsVersions[0] <= version ==> res.0 >= 0
  {
    ...
    if i == -1 then
      ...
      assert ValueSetsVersions[i] >= 0;
      assert MaxMin(version, i, ValueSetsVersions);
      (ValueSetsVersions[i], ValueSets[i])
  }
    ...
    ensures res.0 >= 0 ==> res.0 in ValueSetsVersions
  {
    ...
    if i == -1 then
      ...
      assert MaxMin(version, i, valuesVersions);
